...
- [Model Resolver](https://github.com/Azornes/Comfyui-Model-Resolver) custom node (default on, pinned tag `COMFYUI_MODEL_RESOLVER_REF`): finds a loaded workflow's missing models locally or downloads them from Hugging Face / CivitAI into the model folders. Requirements install under the torch constraints; the run fails if the node does not load. Its frontend imports ComfyUI's `scripts/*.js` with one `../` too many, which breaks it below llama-swap's `/comfyui/` prefix; the task rewrites them to the right depth (a no-op once upstream is fixed) and undoes that before a tag switch. `COMFYUI_MODEL_RESOLVER=false` renames it to `*.disabled`, keeping its settings — API keys entered there are stored in the checkout and returned by its API to anyone who can reach ComfyUI
...
- `COMFYUI_SUPERVISOR=llama-swap`: ComfyUI becomes llama-swap's `comfyui_auto` model — `http://<host>:9292/comfyui/` starts it, requesting any other model stops it, so ComfyUI and an LLM never hold unified memory at the same time. Paths and user are read from the installed llama-swap unit (v249+); the config fragment is validated with `llama-swap -validate` before it is installed; ComfyUI still runs as `COMFYUI_USER` (via `setpriv`), on loopback and the fixed `COMFYUI_PORT` so local tools such as comfy-cli can reach it directly; an outdated loaded ComfyUI is unloaded instead of restarting llama-swap. Switching back to `systemd` removes everything the llama-swap mode installed
- `COMFYUI_MCP=true` (llama-swap mode only): installs the MCP server from `comfyui_mcp/` (pinned `mcp` and `comfy-cli` in its own venv under `/srv/comfyui-mcp`) as llama-swap model `comfyui-mcp`, reachable at `http://<host>:9292/upstream/comfyui-mcp/mcp` with the same API keys as the LLMs. Tools: search nodes, list models, queue a workflow and poll it, show the installed and available ComfyUI releases, and download a model in the background — Hugging Face `/resolve/` or CivitAI download URLs only, into the known model folders only, size known up front and below `COMFYUI_MCP_DOWNLOAD_MAX_GB` (default 50), at least `COMFYUI_MCP_DISK_RESERVE_GB` (default 100) left free, one at a time, never overwriting. Tokens (`HF_API_TOKEN`, `CIVITAI_API_TOKEN`) and those limits go into `/srv/comfyui-mcp/.env`; re-run the task afterwards so llama-swap restarts with them. Requires `apiKeys` in llama-swap; the server gets its own key (`/srv/comfyui-mcp/.env`, passed to llama-swap through a drop-in and appended to `apiKeys` by the `51-comfyui-mcp.yaml` fragment) to load ComfyUI through `/comfyui/`. The fragment is validated together with `config.yaml`, also when switching it off, so a matrix that still names `comfyui-mcp` stops the run before anything changes. You add `comfyui-mcp` to the matrix yourself; the run and `--check` report every set without it and every model in no set, because llama-swap would unload the MCP server mid-call (see `docs/plans/comfyui-mcp-server.md`)
- Idempotent; supports `--check`, `--force` and `--interactive`

**Environment variables (all optional):** `COMFYUI_SUPERVISOR` (`systemd` or `llama-swap`, default `systemd`), `COMFYUI_MCP` (default `false`), `COMFYUI_MCP_DIR` (default `/srv/comfyui-mcp`), `COMFYUI_DIR`, `COMFYUI_REF` (default `v0.35.0`), `COMFYUI_USER`, `COMFYUI_LISTEN` (default `127.0.0.1`), `COMFYUI_PORT` (default `8188`), `COMFYUI_MODELS_DIR`, `COMFYUI_AUTOSTART` (default `false`), `COMFYUI_TORCH_INDEX_URL`, `COMFYUI_RESERVE_VRAM` (default `8`), `COMFYUI_DISABLE_PINNED_MEMORY`, `COMFYUI_EXTRA_ARGS`, `COMFYUI_SAGE_BUILD`, `COMFYUI_SAGE_REF`, `COMFYUI_MODEL_RESOLVER` (default `true`), `COMFYUI_MODEL_RESOLVER_REF` (default `v1.2.1`) — see `--help` for all of them.
