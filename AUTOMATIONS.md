...
- [Model Resolver](https://github.com/Azornes/Comfyui-Model-Resolver) custom node (default on, pinned tag `COMFYUI_MODEL_RESOLVER_REF`): finds a loaded workflow's missing models locally or downloads them from Hugging Face / CivitAI into the model folders. Requirements install under the torch constraints; the run fails if the node does not load. `COMFYUI_MODEL_RESOLVER=false` renames it to `*.disabled`, keeping its settings — API keys entered there are stored in the checkout and returned by its API to anyone who can reach ComfyUI
